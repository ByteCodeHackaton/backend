...
func GetWorkdayList(w http.ResponseWriter, r *http.Request) {
	log.Println("Request workday list..")

	employee_id := r.FormValue("id")
	var message string
	...
	row = db.QueryRowContext(context.Background(), `SELECT Count(*) FROM workdays WHERE employee_id=?;`, employee_id)
	...
	rows, err := db.QueryContext(context.Background(), `SELECT * FROM workdays WHERE employee_id=? LIMIT ? OFFSET ?;`, employee_id, limit_, offset_)
	...
	case nil:
		...
		log.Println("Get workday list successfull!")
	default:
